...
\begin{itemize}
 \item \textbf{Information Discovery:} List objects with available photometry (SED) given a sky position (RA and Dec) and angular size.  Also called a \emph{data discovery} query.
 \item \textbf{Information Availability:} For a given named object (`Target'), how many SED photometric data points are available, if any.
 \item \textbf{Data Retrieval:} For a given named object, return the photometric return the available data in an IVOA Spectrum Data Model compatible VOTable.
\end{itemize}
